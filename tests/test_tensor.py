...
from minitorch import MathTestVariable, Tensor, grad_check, tensor, Parameter, rand
...
def test_vguerra() -> None:
    ...
    def debug(a: Tensor) -> None:
        print(a.name)
        print(a._tensor.to_string(), f"shape={a.shape}")

    layers = 3
    input = tensor([[2, 3], [4, 6], [8, 9], [10, 20], [1, 1]])
    input.name = "input"

    weights = RParam(input.shape[1], layers)
    weights.name = "weights"
    w_value: Tensor = weights.value
    order = list(range(w_value.dims))
    print()
    print(order)
